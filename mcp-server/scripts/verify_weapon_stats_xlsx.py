# Compares `helldivers.raw_weapon_stats` (populated by db/import_raw_sheet.cjs from the live
# DiversDex 2.3 gviz CSV export) against a manually-exported xlsx of the same "Weapon stats" tab,
# cell by cell (percent-formatted columns are normalized numerically so "30%" == 0.3 == "30%").
# Use this whenever a fresher xlsx export is provided to check whether the DB has actually fallen
# behind - it writes any true gaps (xlsx has a value, DB doesn't) to
# mcp-server/cache/weapon_stats_xlsx_patch.json for db/patch_raw_weapon_stats.cjs to apply.
# Update XLSX_PATH below to point at whatever export you were just given.
XLSX_PATH = "D:/dev_study/hldv/weaponstat.xlsx"
...
def cell_to_text(v):
    ...
    if isinstance(v, datetime.datetime):
        # Excel auto-corrupted an "M/D" fraction-looking string into a date - recover it
...
cur.execute(f"SELECT row_num, {', '.join('\"' + c + '\"' for c in db_cols)} FROM raw_weapon_stats ORDER BY row_num")
...
for i in range(min(len(xlsx_rows), len(db_rows))):
    db_row = db_rows[i][1:]  # skip row_num
    xl_row = xlsx_rows[i]
...
with open("D:/dev/helldivers2dex/mcp-server/cache/weapon_stats_xlsx_patch.json", "w", encoding="utf-8") as f:
